...
async function getData(seed) {
  ...
  const rows = await knex('choice')
    ...
    .groupBy('level')
    .select(knex.raw('group_concat(case when is_answer=1 then sentence else "" end, "") as sentence, group_concat(url," ") as urls, group_concat(case when is_answer=1 then url else "" end,"") as answer'))
